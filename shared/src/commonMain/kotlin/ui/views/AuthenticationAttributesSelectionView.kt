            ...
            Surface(
                ...
                Column(
                    ...
                    Row(
                        ...
                        Button(onClick = {
                            val selection = selectedAttributes.entries.associate { it.key to it.value.filter { it.key.value }.values.toSet() }
        ...
        Box(modifier = Modifier.padding(it)) {
            Column(
                ...
                vm.requests.forEach { request ->
                    ...
                    selectedAttributes[request.key] = selections
                    AttributeSelectionGroup(request, selections)
                }
            }
        }
    }
...
@Composable
fun AttributeSelectionGroup(request: Map.Entry<String, Pair<RequestOptionParameters, Map<SubjectCredentialStore. StoreEntry, Map<ConstraintField, NodeList>>>>, selections:  MutableMap<MutableState<Boolean>, NormalizedJsonPath>){
    Column {
        Text(text = request.value.first.credentialIdentifier,
            style = MaterialTheme.typography.labelLarge,
            color = MaterialTheme.colorScheme.secondary,
            fontWeight = FontWeight.SemiBold,)
        request.value.first.attributes?.forEach { attribute ->
            val format = request.value.first.resolved?.first
