...
class DCAPIAuthenticationViewModel(
    ...
    override val descriptors = dcApiRequest.requestedData.mapNotNull {
        DifInputDescriptor(id="org.iso.18013.5.1.mDL", constraints = Constraint(fields = it.value.map { requestedAttribute -> ConstraintField(path = listOf(
            NormalizedJsonPath(
    ...
    override fun findMatchingCredentials(): Map<String, Map<SubjectCredentialStore.StoreEntry, Map<ConstraintField, NodeList /* = List<NodeListEntry> */>>> {
        return runBlocking {
                walletMain.holderAgent.matchInputDescriptorsAgainstCredentialStore(
                    inputDescriptors = descriptors,
                    fallbackFormatHolder = null, // TODO define format probably already in descriptor
                ).getOrThrow()
