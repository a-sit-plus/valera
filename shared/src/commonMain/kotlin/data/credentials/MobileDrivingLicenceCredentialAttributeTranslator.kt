...
class MobileDrivingLicenceCredentialAttributeTranslator : CredentialAttributeTranslator {
    override fun translateSingleClaimReference(claimReference: SingleClaimReference) = when (claimReference) {
        ...
        is MdocClaimReference -> claimLabel(claimReference.namespace, claimReference.claimName)
    }

    override fun translate(attributeName: NormalizedJsonPath): StringResource? =
        attributeName.minus(MDL_NAMESPACE).let {
            it.memberName(0)?.let { claim -> claimLabel(MDL_NAMESPACE, claim) }
        }

    private fun claimLabel(namespace: String, claimName: String): StringResource? =
        if (namespace != MDL_NAMESPACE) null else with(MobileDrivingLicenceDataElements) {
            when (claimName) {
