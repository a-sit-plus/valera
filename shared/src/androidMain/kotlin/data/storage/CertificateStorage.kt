...
object CertificateStorage {
    ...
    private fun saveCertificate(context: Context, alias: String, pemCertificate: String) {
        val preferences = getPreferences(context)
        preferences.edit().putString(alias, pemCertificate).apply()
    }

    fun loadCertificate(context: Context, alias: String): X509Cert? {
        val preferences = getPreferences(context)
        val pemCertificate = preferences.getString(alias, null) ?: return null
        return X509Cert.fromPem(pemCertificate)
    }

    private fun pemToX509Certificate(pemData: String): X509Certificate? {
